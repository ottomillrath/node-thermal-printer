...
module.exports = {
    ...
    CTL_SET_HT:  Buffer.from([]), //Buffer.from([0x1b, 0x44]), // Set horizontal tab positions
    CTL_VT: Buffer.from([]), //Buffer.from([0x1b, 0x66, 0x31, 0x01]), // Vertical tab
  
    // Printer hardware
    HW_INIT: Buffer.from([0x1b, 0x40]), // Clear data in buffer and reset modes
    HW_SELECT: Buffer.from([]), //Buffer.from([0x1b, 0x3d, 0x01]), // Printer select
    HW_RESET: Buffer.from([0x1b, 0x40]), //Buffer.from([0x1b, 0x3f, 0x0a, 0x00]), // Reset printer hardware
    ...
    // Paper
    PAPER_FULL_CUT: Buffer.from([0x1b, 0x77]), // Full cut paper
    PAPER_PART_CUT: Buffer.from([0x1b, 0x6D]), // Partial cut paper
  
    // Text format
    TXT_NORMAL: Buffer.from([0x1b, 0x21, 0x00]), //Buffer.from([0x1b, 0x21, 0x08]), // Normal text
    TXT_2HEIGHT: Buffer.from([]), //Buffer.from([0x1b, 0x21, 0x04]), // Double height text
    TXT_2WIDTH: Buffer.from([]), //Buffer.from([0x1b, 0x21, 0x05]), // Double width text
    TXT_4SQUARE: Buffer.from([]), //Buffer.from([0x1b, 0x21, 0x06]), // Quad area text
    TXT_UNDERL_OFF: Buffer.from([]), //Buffer.from([0x1b, 0x2d, 0x00]), // Underline font OFF
    TXT_UNDERL_ON: Buffer.from([]), //Buffer.from([0x1b, 0x2d, 0x01]), // Underline font 1-dot ON
    TXT_UNDERL2_ON: Buffer.from([]), //Buffer.from([0x1b, 0x2d, 0x02]), // Underline font 2-dot ON
    TXT_BOLD_OFF: Buffer.from([0x1B, 0x46]), // Bold font OFF
    TXT_BOLD_ON: Buffer.from([0x1B, 0x45]), // Bold font ON
    TXT_INVERT_OFF:Buffer.from([]), // Buffer.from([0x1d, 0x42, 0x00]), // Invert font OFF (eg. white background)
    TXT_INVERT_ON:Buffer.from([]), // Buffer.from([0x1d, 0x42, 0x01]), // Invert font ON (eg. black background)
    TXT_FONT_A: Buffer.from([0x1B, 0x48]), // Font type A
    TXT_FONT_B: Buffer.from([0x1B, 0x0F]), // Font type B
    TXT_ALIGN_LT: Buffer.from([0x1B, 0x61, 0x30]), // Left justification
    TXT_ALIGN_CT: Buffer.from([0x1B, 0x61, 0x31]), // Centering
    TXT_ALIGN_RT: Buffer.from([0x1B, 0x61, 0x32]), // Right justification
  
    // All code pages supported by printer.
    CODE_PAGE_PC437_USA: Buffer.from([0x1b, 0x74, 0]),
    CODE_PAGE_KATAKANA: Buffer.from([0x1b, 0x74, 1]),
    CODE_PAGE_PC850_MULTILINGUAL: Buffer.from([0x1B, 0x74, 0x32]),
    ...
    QRCODE_CORRECTION_L: Buffer.from([0x30]), // Correction level: L - 7%
    QRCODE_CORRECTION_M: Buffer.from([0x31]), // Correction level: M - 15%
    QRCODE_CORRECTION_Q: Buffer.from([0x32]), // Correction level: Q - 25%
    QRCODE_CORRECTION_H: Buffer.from([0x33]), // Correction level: H - 30%
  
    QRCODE_CELLSIZE_1: Buffer.from([0x01]), // Cell size 1
    QRCODE_CELLSIZE_2: Buffer.from([0x02]), // Cell size 2
    QRCODE_CELLSIZE_3: Buffer.from([0x03]), // Cell size 3
    QRCODE_CELLSIZE_4: Buffer.from([0x04]), // Cell size 4
    QRCODE_CELLSIZE_5: Buffer.from([0x05]), // Cell size 5
    QRCODE_CELLSIZE_6: Buffer.from([0x06]), // Cell size 6
    QRCODE_CELLSIZE_7: Buffer.from([0x07]), // Cell size 7
    QRCODE_CELLSIZE_8: Buffer.from([0x08]), // Cell size 8
